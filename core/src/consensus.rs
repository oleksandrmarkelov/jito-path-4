...
impl Default for Tower {
    fn default() -> Self {
        let mut tower = Self {
            ...
            mostly_confirmed_threshold: Option::default(),
            threshold_ahead_count: Option::default(),
            after_skip_threshold: Option::default(),
            threshold_escape_count: Option::default(),
            last_config_check_seconds: u64::default(),
        };
...
impl Tower {
    ...
    pub fn update_config(&mut self) {
        ...
        if config_check_seconds >= (self.last_config_check_seconds + 60) {
            ...
            // a is threshold - no slot that hasn't already achieved this vote weight will be voted on
            // b is "vote ahead of threshold" - how many slots ahead to vote, regardless of weight
            // c controls what stake weighted vote percentage is required after skips
            // d is "escape hatch distance" - slots of non-voting before voting anyway
    ...
    pub fn pop_votes_locked_out_at(&self, new_votes: &mut Vec<Slot>, slot: Slot) {
        ...
        for i in 0..new_votes.len() {
            Self::process_slot_vote_unchecked(&mut vote_state, new_votes[i]);
    ...
    pub fn is_locked_out_including(
        ...
        for slot in including {
            Self::process_slot_vote_unchecked(&mut vote_state, *slot);
        }
    ...
    fn process_slot_vote_unchecked(vote_state: &mut TowerVoteState, slot: Slot) {
        let _ = Self::process_vote_unchecked(vote_state, Vote::new(vec![slot], Hash::default()), true);
    }

    fn process_vote_unchecked(
        vote_state: &mut TowerVoteState,
        vote: Vote,
        pop_expired: bool,
    ) -> Result<()> {
        if vote.slots.is_empty() {
            return Err(TowerError::FatallyInconsistent(
                "this is wrong patch in process_vote_unchecked",
            ));
        }
        let slot_hashes: Vec<_> = vote.slots.iter().rev().map(|x| (*x, vote.hash)).collect();
        Self::process_vote_unfiltered(
            vote_state,
            &vote.slots,
            &vote,
            &slot_hashes,
    ...
    fn process_vote_unfiltered(
        ...
        Self::check_slots_are_valid(vote_state, vote_slots, &vote.hash, slot_hashes)?;
        vote_slots
            .iter()
            .for_each(|s| vote_state.process_next_vote_slot(*s, pop_expired));
        Ok(())
    }
    ...
    fn check_slots_are_valid(
        ...
        while i < vote_slots.len() && j > 0 {
                ...
                .is_some_and(|last_voted_slot| vote_slots[i] <= last_voted_slot)
            {
                i = i
                    .checked_add(1)
                    .expect("`i` is bounded by `MAX_LOCKOUT_HISTORY`");
                continue;
            }
        ...
        if j == slot_hashes.len() {
            return Err(TowerError::FatallyInconsistent("vote too old"));
        }
        if i != vote_slots.len() {
            return Err(TowerError::FatallyInconsistent("slots mismatch"));
        }
        if &slot_hashes[j].1 != vote_hash {
            return Err(TowerError::FatallyInconsistent("slot hash mismatch"));
        }
        Ok(())
    }
